...
DROP TABLE MANIFIESTA;
CREATE TABLE MANIFIESTA(
paciente_codigo number(4),
sintoma_codigo number(4)
) TABLESPACE triaje;
...
inicio3 number(3),
inicio4 number(3),
-- La Localización es un atributo difuso tipo 2 dado que en algunas oportunidades debe tomar el valor UNDEFINED, en
-- el resto de los casos tomara un valor crisp.
localizaciont number(1) default 1 not null,
localizacion1 number(3),
localizacion2 number(3),
localizacion3 number(3),
localizacion4 number(3),
...
ALTER TABLE MANIFIESTA
add CONSTRAINT manifiesta_paciente_fk
FOREIGN KEY (paciente_codigo)
REFERENCES PACIENTES(codigo);

ALTER TABLE MANIFIESTA
add CONSTRAINT manifiesta_sintoma_fk
FOREIGN KEY (sintoma_codigo)
REFERENCES SINTOMAS(codigo);
...
BEGIN
  ...
  SELECT column_id into c_localizacion
    FROM user_tab_columns WHERE table_name='SINTOMAS' AND column_name='LOCALIZACIONT';	
...
-- --------------------------------------------------------------------------------------   
  ...
  INSERT into FCL values (t_SINTOMAS,c_localizacion,2,1,USER||'.SINTOMAS.LOCALIZACION');
...
-- --------------------------------------------------------------------------------------  
  ...
  INSERT into FOL values (t_CONSTANTES,c_nihss,1,'somnolencia',0);
  INSERT into FOL values (t_CONSTANTES,c_nihss,2,'estupor',0);
...
-- --------------------------------------------------------------------------------------  
  ...
  -- Tabla Sintomas
  INSERT into FLD values (t_SINTOMAS,c_inicio_s,0,0,8,18,24);
  INSERT into FLD values (t_SINTOMAS,c_inicio_s,1,20,48,72,96);
...
*/
  ...
  INSERT into FLD values (t_CONSTANTES,c_nihss,1,3,4,5,6);     --'somnolencia',0);
  INSERT into FLD values (t_CONSTANTES,c_nihss,2,6,7,8,9);     -- 'estupor',0);
...
*/
  ...
  INSERT INTO FLD values (t_SIGNOS,c_pulso_radial,3,7,8,9,10);    -- 'fuerte',1);
  INSERT INTO FLD values (t_SIGNOS,c_pulso_radial,2,10,11,12,13); -- 'normal',1);
